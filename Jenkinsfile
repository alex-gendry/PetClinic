        ...
        stage('fod') {
            steps {
                container('sast-client') {
                    ...
                    // Create appversion
                    sh 'fcli ssc appversion create $APP_VERSION --auto-required-attrs --skip-if-exists'
                    ...
                    // Run Scan
                    sh "fcli sc-sast scan start -p package.zip --sensor-version 22.2 --appversion $APP_VERSION --store '?'"
